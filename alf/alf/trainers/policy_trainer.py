...
from alf.bin.grid_value import grid_draw
from alf.bin.layer_value import layer_draw
...
class RLTrainer(Trainer):
    ...
    def _train(self):
        ...
        if self._num_iterations:
            time_to_checkpoint = self._trainer_progress._iter_num + checkpoint_interval  # iter
        else:
            time_to_checkpoint = self._trainer_progress._env_steps + checkpoint_interval  # steps
...
def play(root_dir,
    ...
    while episodes < num_episodes:
        ...
        if 'map' in root_dir and not time_step.is_first():  # and not time_step.is_first()
            x, y = time_step.observation.squeeze()[0:2]
            if hasattr(policy_state, 'skill_generator'):
                o = policy_step.state.skill_generator.skill.cpu().numpy()[0]
                logging.log_every_n_seconds(
                    logging.INFO, 'step %0.0f -> %0.2f %0.2f -> %s' % (episode_length, x, y, np.around(o, 3)), n_seconds=0)
            else:
                logging.log_every_n_seconds(
                    logging.INFO, 'step %0.0f -> %0.2f %0.2f' % (episode_length, x, y), n_seconds=0)

        elif 'fetch' in root_dir:  # and not time_step.is_first()
            x, y, z = time_step.observation.squeeze()[0:3]
            if hasattr(policy_state, 'skill_generator'):
                o = policy_step.state.skill_generator.skill.cpu().numpy()[0]
                logging.log_every_n_seconds(
                    logging.INFO, 'step %0.0f -> %0.3f %0.3f %0.3f -> %s' % (episode_length, x, y, z, np.around(o, 3)), n_seconds=0)
            else:
                logging.log_every_n_seconds(
                    logging.INFO, 'step %0.0f -> %0.3f %0.3f %0.3f' % (episode_length, x, y, z), n_seconds=0)

            if rgb:
                env_name = root_dir[root_dir.rindex('/') + 1:]
                rgb_dir = f"log/{env_name}"
                if not os.path.exists(rgb_dir): os.makedirs(rgb_dir)

                plt.figure(figsize=(5, 5), dpi=1200)
                img = env.render(mode='rgb_array')
                plt.imsave(f'{rgb_dir}/rbg_{episode_length}.png', img, dpi=1200)

        if time_step.is_first() and 'fetch' in root_dir:
            obs = env._env._env.gym.env.env.env.env.env.env._get_obs()
            # achieved_goal = np.around(obs['achieved_goal'], 3)  # np
            # desired_goal = np.around(obs['desired_goal'], 3)
            # logging.info('achieved_goal: %s, desired_goal: %s' % (achieved_goal, desired_goal))

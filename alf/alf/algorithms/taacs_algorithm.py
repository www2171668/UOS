...
@alf.configurable
class TaacsAlgorithm(OffPolicyAlgorithm):
    ...
    def _compute_beta_and_action(self, observation, state, epsilon_greedy, mode):
        ...
        b1_action_dist, _ = self._actor_network(observation)  # π(a^|s)
        if mode == Mode.predict:  # * a ～ a_dist
            b1_action = dist_utils.epsilon_greedy_sample(b1_action_dist, epsilon_greedy)  # 贪婪
        else:
            b1_action = dist_utils.rsample_action_distribution(b1_action_dist)  # 重采样

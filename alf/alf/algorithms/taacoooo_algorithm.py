...
TaacLossInfo = namedtuple('TaacLossInfo', ["actor", "critic", "alpha", "option"])
...
class TaacAlgorithm(OffPolicyAlgorithm):
    ...
    def __init__(self,
                 ...
                 option_network_cls=ActorDistributionNetwork,
                 reward_weights=None,
        ...
        Args:
            ...
            option_network_cls (Callable): (Q1, Q2)->离散分布 ～ b1 or b2.
                b2对应b. 网络为 (64, 2)
        ...
        option_spec = BoundedTensorSpec((), dtype="int64", maximum=1)  # 选择a^的概率
        ...
        critic_networks, actor_network, option_network = self._make_networks_impl(
            observation_spec, action_spec, option_spec,
            actor_network_cls, critic_network_cls, option_network_cls)
        ...
        self._target_entropy = nest.map_structure(
            ...
            target_entropy, (option_spec, action_spec))
    ...
    # % 创建actor和cirtic网络
    def _make_networks_impl(self, observation_spec, action_spec, option_spec,
                            actor_network_cls, critic_network_cls, option_network_cls):
        ...
        actor_network = actor_network_cls(
            ...
            input_preprocessors=(alf.layers.Detach(), action_embedding),  # 预处理器,状态不变,将动作维度扩展到状态维度
        ...
        option_embedding = torch.nn.Sequential(alf.layers.FC(input_size=action_spec.numel,
                                                             output_size=observation_spec.numel))
        option_network = option_network_cls(
            input_tensor_spec=(observation_spec, action_spec, action_spec),
            input_preprocessors=(alf.layers.Detach(), option_embedding, option_embedding),
            preprocessing_combiner=nest_utils.NestConcat(),
            action_spec=option_spec)
    ...
    def _compute_beta_and_action(self, observation, state, epsilon_greedy, mode):
        ...
        b1_action_dist, _ = self._actor_network((observation, state.action))  # π(a^|s,a-)
        if mode == Mode.predict:  # * a ～ a_dist
            b1_action = dist_utils.epsilon_greedy_sample(b1_action_dist, epsilon_greedy)  # a^ 与action无关
        else:
        ...
        # % 3. 计算β分布
        beta_dist, _ = self._option_network((observation, b0_action, b1_action))  # (s, a-, a) -> β  β[1]表示中断o-的概率
    ...
    def _predict_action(self, observation, state, epsilon_greedy=None, mode=Mode.rollout):
        ...
        if not common.is_eval() and not self._training_started:
            b = self._option_spec.sample(observation.shape[:1])  # b
    ...
    def _compute_critics(self, critic_net, observation, action,
        ...
        critics, _ = critic_net((observation, action))  # [B, replicas]
        if replica_min:
    ...
    def train_step(self, inputs: TimeStep, state: TaacState, rollout_info: TaacInfo):
        ...
        option_loss = self._option_train_step(inputs, beta_dist, q_values2)
    ...
    def _actor_train_step(self, a, b1_a_entropy, beta_dist, beta_entropy, q_values2):
        alpha = self._log_alpha[1].exp().detach()  # logπ'^的温度系数 tensor(1.)
        q_a = beta_dist.probs[:, 1].detach() * q_values2[:, 1]  # β(b1) * Q(s,a^)
    ...
    def _option_train_step(self, inputs, beta_dist, q_values2):
        """Termination loss   β(b1) * (Q(s,a^) - maxQ(s) + reg) * (1-done)"""
        is_lasts = (inputs.step_type == StepType.LAST).to(dtype=torch.float32)
        is_lasts = common.expand_dims_as(is_lasts, q_values2[:,1])

        option_loss = beta_dist.probs[:, 0].detach() * (q_values2[:, 1] - torch.max(q_values2, 1).values)
        option_loss = (option_loss + self._termination_reg).detach() * (1 - is_lasts)
        return option_loss
    ...
    def calc_loss(self, info: TaacInfo):
        ...
        loss = actor_loss.loss + alpha_loss + critic_loss.loss + option_loss
        extra = TaacLossInfo(actor=actor_loss.extra, critic=critic_loss.extra, alpha=alpha_loss, option=option_loss)

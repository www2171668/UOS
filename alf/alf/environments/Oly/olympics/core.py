...
class OlympicsBase(object):
    def __init__(self, map, seed=None, gamma=0.98, tau=0.01):
        """gamma: v衰减系数,其惯性作用
           tau: delta t,力的系数.实际作用于状态时,还会×0.1. 例如100的力,只能移动状态1的距离
           gamma和tua必须配合使用,例如0.98的惯性和0.1的折扣.
           当gamma=0时,可能需要1的折扣,否则移动距离太小
        """
        self.oly_seed = seed
        self.set_seed()

        self.action_f = [-100, 200]  # 力的边界
        self.action_theta = [-30, 30]  # 角度边界
        ...
        # env hyper
        self.gamma = gamma  # v衰减系数,其惯性作用
        self.tau = tau  # delta t,力的系数.实际作用于状态时,还会×0.1.
        self.restitution = 0.8

        self.step_cnt = 0
        self.done = False
        self.max_step = 500  # 最大步数
    ...
    def update_all(self, pos_container, v_container, t, a):
        """更新状态和速度"""
        for agent_idx in range(self.agent_num):
            accel_x, accel_y = a[agent_idx]  # 矢量力

            pos_old = pos_container[agent_idx]  # 当前位置
            v_old = v_container[agent_idx]  # 当前角度
            ...
            vx_new = self.gamma * vx + self.tau * accel_x  # update v with acceleration
            vy_new = self.gamma * vy + self.tau * accel_y
            v_new = [vx_new, vy_new]
    ...
    def circle_collision_time(self, pos_container, v_container, remaining_t, ignore):
    ...
    def actions_to_accel(self, actions_list):
        """Convert action(force) to acceleration, 即从标量力转为矢量力;
        if the action is None, the accel is zero;
        if the agent is fatigue, the force will convert to no acceleration;
        """
        ...
        for agent_idx in range(self.agent_num):
            ...
            if action is None:
                ...
                if self.agent_list[agent_idx].is_fatigue:  # if agent is out of energy, no driving force applies
                    ...
                    assert self.action_f[0] <= action[0] <= self.action_f[1], \
                        print('Continuous driving force needs to be within the range [-100,200]')
                    ...
                    mass = self.agent_list[agent_idx].mass  # 质量
                    force = action[0] / mass  # 标量力
                    theta = action[1]  # 角度

                    theta_old = self.agent_theta[agent_idx][0]  # 原角度
                    theta_new = theta_old + theta  # 新角度
                    self.agent_theta[agent_idx][0] = theta_new  # 赋值

                    accel_x = force * math.cos(theta_new / 180 * math.pi)
                    accel_y = force * math.sin(theta_new / 180 * math.pi)
                    accel = [accel_x, accel_y]  # 矢量力
    ...
    def stepPhysics(self, actions_list, step=None):
        ...
        temp_a_container = self.actions_to_accel(actions_list)  # 将动作转换为加速度(矢量力)
        ...
        while True:
            ...
            if collision_wall_target is not None and collision_circle_target is None:
                ...
                temp_pos_container, temp_v_container = \
                    self.update_all(temp_pos_container, temp_v_container, remaining_t, temp_a_container)
    ...
    def change_inner_state(self):
        ...
        for agent_idx in range(self.agent_num):
            if self.agent_list[agent_idx].energy < 0:  # once fatigue, the agent died and lose control
                ...
                consume_rate = force * v / 50  # 消耗精力
                consume_rate -= self.energy_recover_rate

                remaining_energy = self.agent_list[agent_idx].energy - consume_rate * self.tau  # 剩余精力
    ...
    def render(self, info=None):
        ...
        if self.show_traj:
            self.get_trajectory()
            self.viewer.draw_trajectory(self.agent_record, self.agent_list)
        self.viewer.draw_direction(self.agent_pos, self.agent_accel)  # 画出智能体上的方向线
